...
# the eks stack). null = no certificate = no TLS listeners allowed.
...
module "nlb" {
  ...
  # Internet-facing NLBs live in the public subnets, internal ones with
  # the workloads. Terraform places the NLB itself, so no
  ...
  # lands there; the module opens it to the NLB per target port.
