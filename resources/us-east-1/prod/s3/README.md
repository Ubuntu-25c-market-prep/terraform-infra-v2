...
(`prod-s3-us-east-1-velero-<account-id>`); the account id is appended at
...
2. Open a PR with `- Path: /resources/us-east-1/prod/s3` in the commit
...
| Key | Meaning |
|---|---|
| `name` | the bucket name prefix, `<env>-s3-<region>` (`prod-s3-us-east-1`); every bucket is created as `<name>-<bucket>-<account-id>`. Changing it recreates every bucket |
