...
| Key | Meaning |
|---|---|
| `name` | `<env>-bastion-<region>` (`uat-bastion-us-east-1`): security group, key pair and instance role name; each instance states its own full name |
...
| `ssh_public_key` | the team's **public** key; Terraform creates the key pair named `name` from it. The private half lives in SSM Parameter Store `/uat/bastion/ssh-private-key` |
