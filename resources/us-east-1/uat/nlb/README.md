...
- The AWS Load Balancer Controller addon (Flux) with its IRSA role
  (`uat-irsa-aws-load-balancer-controller-us-east-1` in `../eks/iam.yaml`).
...
| Key | Meaning |
|---|---|
| `name` | the full NLB name, `<env>-nlb-<region>` (`uat-nlb-us-east-1`); also the NLB SG name and the target group prefix (32-char limit overall) |
