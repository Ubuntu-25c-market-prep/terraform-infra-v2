...
| Key | Meaning |
|---|---|
| `name` | the full ALB name, `<env>-alb-<region>` (`dev-alb-us-east-1`); also the ALB SG name and the target group prefix. Target group names are `<name>-<entry name>` and limited to 32 characters, so 14 remain for the entry name - it is a label, not the Service name |
